...
#[test]
fn a_key_rastro_has_never_seen_does_not_fail_the_read() {
    // Arrange: RabbitMQ adds keys between releases, and 3.13 carried
    // `release_series_support_status`, which 4.0 dropped again.
    let unfamiliar = MEASURED.replace(
        r#""os": "Linux","#,
        r#""os": "Linux", "something_a_later_release_added": {"nested": true},"#,
    );

    // Act & Assert: an upgrade of a box nobody touched must not fail this facet, so unknown
    // fields are ignored rather than refused.
    let status = RabbitmqctlStatus::parse(&unfamiliar).expect("unknown keys are ignored");
    assert_eq!(status.rabbitmq_version, "4.0.5");
}

#[test]
fn a_key_a_supported_release_stopped_sending_is_absent_rather_than_fatal() {
    // Arrange: the mirror image, and the one that actually bites — a field rastro reads that
    // a release removes.
    let without = MEASURED.replace(r#""raft_data_directory""#, r#""raft_data_directory_gone""#);

    // Act
    let status = RabbitmqctlStatus::parse(&without).expect("a missing optional field is absent");
