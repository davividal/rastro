...
//!
//! **Past it, and no further.** The document starts at the first line beginning with `{`, so
//! a preamble is skipped whole lines at a time rather than by hunting for a brace that an
//! Erlang term in a report could perfectly well contain. Output with no such line is handed
//! back unchanged, so the caller's own error carries what the tool actually said.
